...
var _ plugin.ActionProvider = (*ModelCatalogPlugin)(nil)

// HandleAction executes an action on a source or asset.
func (p *ModelCatalogPlugin) HandleAction(ctx context.Context, scope plugin.ActionScope, targetID string, req plugin.ActionRequest) (*plugin.ActionResult, error) {
	switch scope {
...
func (p *ModelCatalogPlugin) ListActions(scope plugin.ActionScope) []plugin.ActionDefinition {
	switch scope {
	case plugin.ActionScopeSource:
...
func (p *ModelCatalogPlugin) handleSourceAction(ctx context.Context, sourceID string, req plugin.ActionRequest) (*plugin.ActionResult, error) {
	switch req.Action {
	case "refresh":
...
func (p *ModelCatalogPlugin) handleAssetAction(ctx context.Context, entityName string, req plugin.ActionRequest) (*plugin.ActionResult, error) {
...
func (p *ModelCatalogPlugin) builtinActionHandler() *plugin.BuiltinActionHandler {
	if p.cfg.DB == nil {
		return nil
	}
	store := plugin.NewOverlayStore(p.cfg.DB)
	return plugin.NewBuiltinActionHandler(store, PluginName, "CatalogModel")
}
