...
var (
	_ plugin.SourceManager          = (*PromptTemplatePlugin)(nil)
	_ plugin.RefreshProvider        = (*PromptTemplatePlugin)(nil)
	_ plugin.DiagnosticsProvider    = (*PromptTemplatePlugin)(nil)
	_ plugin.CapabilitiesProvider   = (*PromptTemplatePlugin)(nil)
	_ plugin.CapabilitiesV2Provider = (*PromptTemplatePlugin)(nil)
	_ plugin.UIHintsProvider        = (*PromptTemplatePlugin)(nil)
	_ plugin.CLIHintsProvider       = (*PromptTemplatePlugin)(nil)
	_ plugin.BasePathProvider       = (*PromptTemplatePlugin)(nil)
)

// GetCapabilitiesV2 returns the full V2 capabilities discovery document.
func (p *PromptTemplatePlugin) GetCapabilitiesV2() plugin.PluginCapabilitiesV2 {
	basePath := "/api/prompts_catalog/v1alpha1"
		...
		Entities: []plugin.EntityCapabilities{
			{
				...
				Description: "Reusable prompt templates for AI models",
				Endpoints: plugin.EntityEndpoints{
					List: basePath + "/prompttemplates",
					Get:  basePath + "/prompttemplates/{name}",
				},
				Fields: plugin.EntityFields{
					Columns: []plugin.V2ColumnHint{
						...
						{Name: "format", DisplayName: "Format", Path: "format", Type: "string", Sortable: true, Width: "md"},
						{Name: "taskTags", DisplayName: "Task Tags", Path: "taskTags", Type: "array", Width: "md"},
					...
					FilterFields: []plugin.V2FilterField{
						...
						{Name: "format", DisplayName: "Format", Type: "select", Options: []string{"chat_messages", "completion", "jinja2", "handlebars", "raw"}, Operators: []string{"=", "!="}},
						{Name: "taskTags", DisplayName: "Task Tags", Type: "text", Operators: []string{"=", "!=", "LIKE"}},
						{Name: "author", DisplayName: "Author", Type: "text", Operators: []string{"=", "!=", "LIKE"}},
					},
					DetailFields: []plugin.V2FieldHint{
						...
						{Name: "format", DisplayName: "Format", Path: "format", Type: "string", Section: "Overview"},
						...
						{Name: "template", DisplayName: "Template", Path: "template", Type: "string", Section: "Template"},
						{Name: "parametersSchema", DisplayName: "Parameters Schema", Path: "parametersSchema", Type: "object", Section: "Parameters"},
						{Name: "outputSchema", DisplayName: "Output Schema", Path: "outputSchema", Type: "object", Section: "Parameters"},
						{Name: "modelConstraints", DisplayName: "Model Constraints", Path: "modelConstraints", Type: "object", Section: "Model Constraints"},
					},
				},
				UIHints: &plugin.EntityUIHints{
					Icon:           "file-code",
					NameField:      "name",
					DetailSections: []string{"Overview", "Template", "Parameters", "Model Constraints"},
				},
...
func (p *PromptTemplatePlugin) Capabilities() plugin.PluginCapabilities {
	return plugin.PluginCapabilities{
		EntityKinds:  []string{"PromptTemplate"},
		ListEntities: true,
		GetEntity:    true,
		ListSources:  true,
	}
...
func (p *PromptTemplatePlugin) ListSources(ctx context.Context) ([]plugin.SourceInfo, error) {
...
func (p *PromptTemplatePlugin) ValidateSource(ctx context.Context, src plugin.SourceConfigInput) (*plugin.ValidationResult, error) {
...
func (p *PromptTemplatePlugin) ApplySource(ctx context.Context, src plugin.SourceConfigInput) error {
	...
	if src.Type == "yaml" {
		...
		if err != nil {
			return fmt.Errorf("failed to load prompt template source: %w", err)
		}
		for i := range entries {
...
func (p *PromptTemplatePlugin) EnableSource(ctx context.Context, id string, enabled bool) error {
	p.mu.Lock()
	defer p.mu.Unlock()
...
func (p *PromptTemplatePlugin) DeleteSource(ctx context.Context, id string) error {
	p.mu.Lock()
	defer p.mu.Unlock()
...
func (p *PromptTemplatePlugin) Refresh(ctx context.Context, sourceID string) (*plugin.RefreshResult, error) {
	start := time.Now()
...
func (p *PromptTemplatePlugin) RefreshAll(ctx context.Context) (*plugin.RefreshResult, error) {
	start := time.Now()
	totalLoaded := 0
...
func (p *PromptTemplatePlugin) Diagnostics(ctx context.Context) (*plugin.PluginDiagnostics, error) {
...
func (p *PromptTemplatePlugin) UIHints() plugin.UIHints {
	return plugin.UIHints{
		...
		ListColumns: []plugin.ColumnHint{
			...
			{Field: "format", Label: "Format", Sortable: true, Filterable: true},
			{Field: "taskTags", Label: "Task Tags", Sortable: false, Filterable: true},
		...
		DetailFields: []plugin.FieldHint{
			...
			{Field: "taskTags", Label: "Task Tags"},
			{Field: "version", Label: "Version"},
			{Field: "author", Label: "Author"},
			{Field: "license", Label: "License"},
			{Field: "parametersSchema", Label: "Parameters Schema"},
			{Field: "outputSchema", Label: "Output Schema"},
			{Field: "modelConstraints", Label: "Model Constraints"},
		},
	}
}

// CLIHints returns display hints for the CLI.
func (p *PromptTemplatePlugin) CLIHints() plugin.CLIHints {
	return plugin.CLIHints{
		DefaultColumns:   []string{"name", "format", "taskTags", "version", "author"},
		SortField:        "name",
		FilterableFields: []string{"name", "format", "taskTags", "author", "license"},
	}
}
