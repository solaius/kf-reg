...
var (
	_ plugin.ActionProvider = (*McpServerCatalogPlugin)(nil)
)

// HandleAction executes an action on a source or asset.
func (p *McpServerCatalogPlugin) HandleAction(ctx context.Context, scope plugin.ActionScope, targetID string, req plugin.ActionRequest) (*plugin.ActionResult, error) {
	switch scope {
...
func (p *McpServerCatalogPlugin) ListActions(scope plugin.ActionScope) []plugin.ActionDefinition {
	switch scope {
	case plugin.ActionScopeSource:
...
func (p *McpServerCatalogPlugin) handleSourceAction(ctx context.Context, sourceID string, req plugin.ActionRequest) (*plugin.ActionResult, error) {
	switch req.Action {
	case "refresh":
...
func (p *McpServerCatalogPlugin) handleAssetAction(ctx context.Context, entityName string, req plugin.ActionRequest) (*plugin.ActionResult, error) {
...
// creating one lazily using the shared DB from plugin config.
func (p *McpServerCatalogPlugin) builtinActionHandler() *plugin.BuiltinActionHandler {
	if p.cfg.DB == nil {
		return nil
	}
	store := plugin.NewOverlayStore(p.cfg.DB)
	return plugin.NewBuiltinActionHandler(store, PluginName, "McpServer")
}
