...
func TestGovernanceStore_CRUD(t *testing.T) {
	...
	// Get the record.
	got, err := store.Get("mcp", "mcpserver", "filesystem")
	require.NoError(t, err)
	...
	// Delete the record.
	err = store.Delete("mcp", "mcpserver", "filesystem")
	require.NoError(t, err)
...
func TestGovernanceStore_EnsureExists(t *testing.T) {
	...
	record, err := store.EnsureExists("mcp", "mcpserver", "filesystem", "", "alice")
	require.NoError(t, err)
	...
	record2, err := store.EnsureExists("mcp", "mcpserver", "filesystem", "", "bob")
	require.NoError(t, err)
	...
	// With explicit UID.
	record3, err := store.EnsureExists("model", "model", "llama", "custom-uid-123", "charlie")
	require.NoError(t, err)
...
func TestGovernanceStore_List(t *testing.T) {
	...
	records, nextToken, err := store.List("mcp", 10, "")
	require.NoError(t, err)
	assert.Len(t, records, 5)
	assert.Empty(t, nextToken)

	// Paginate with pageSize 2.
	page1, token1, err := store.List("mcp", 2, "")
	require.NoError(t, err)
	assert.Len(t, page1, 2)
	assert.NotEmpty(t, token1)

	page2, token2, err := store.List("mcp", 2, token1)
	require.NoError(t, err)
	assert.Len(t, page2, 2)
	assert.NotEmpty(t, token2)

	page3, token3, err := store.List("mcp", 2, token2)
	require.NoError(t, err)
	assert.Len(t, page3, 1)
	assert.Empty(t, token3)

	// List for "model" plugin.
	modelRecords, _, err := store.List("model", 10, "")
	require.NoError(t, err)
	assert.Len(t, modelRecords, 1)

	// List for non-existent plugin.
	emptyRecords, _, err := store.List("nonexistent", 10, "")
	require.NoError(t, err)
